...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    x1_norm_sq = torch.square(x_1).sum(dim=1, keepdim=True)
    x2_norm_sq = torch.square(x_2).sum(dim=1, keepdim=False)
    pairwise_scalar_prod = torch.matmul(x_1, x_2.T)
    dists = x1_norm_sq + x2_norm_sq - 2 * pairwise_scalar_prod
    rbf_dists = torch.exp(dists / -2 / sigma**2)
    return torch.Tensor(rbf_dists).type(torch.float32)
...
def hinge_loss(scores, labels):
    ...
    return torch.max(1 - scores * labels, torch.tensor(0)).mean()
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                preds = preds.flatten() # like np.ravel()
    ...
    def predict_scores(self, batch):
        with torch.no_grad(): # ???
            ...
            K = self.kernel_function(batch, self.X) # (batch_size, n_obj)
